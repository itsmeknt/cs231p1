...
a2=params(1);       % quadratic coefficient for squared distance in x
b2=params(2);       % quadratic coefficient for linear distance in x
a1=params(3);       % quadratic coefficient for squared distance in y
b1=params(4);       % quadratic coefficient for linear distance in y
...
scores=zeros(n,m);
for j=1:n
    [dummyScores(:,j) dummyPos(:,j)]=distTransform(conv(:,j),[a1 b1]);
end
positions=zeros(m,n,2);
for i=1:m
    [scores(:,i) x_positions(:,i)]=distTransform(dummyScores(i,:),[a2 b2]);
