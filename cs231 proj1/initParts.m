...
for h=3:1:size(rootWeights,1)-2
    for w=3:1:size(rootWeights,2)-2
        if (w*h>=1*rootArea/numParts && w*h<=1.2*rootArea/numParts && abs(h-w)<=4)   % constraints
            numSizes=numSizes+1;
...
model.numparts = max(model.numparts, partIdx);
...
if ~fake
    ...
    model.components{componentIdx}.numblocks = model.components{componentIdx}.numblocks+1;
    ...
    model.components{componentIdx}.dim = model.components{componentIdx}.dim + 2 + numel(model.defs{cumIdx}.w) + width*size(validSizes{i},1)*31;
end
partnerIdx = cumIdx;
end
